...
.h1-header-open + .foldable-content {
    max-height: 1000px; /* adjust this value based on your content's typical height */
    opacity: 1;
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  headers.forEach(function(header) {
    header.classList.add('h1-header');
...
color_dat <- tibble(
  colors = c("#f68f2d", "#3db53a", "#263778",
             "#851a64", "#122746", "#e6001a", 
             "#ffd900"),
  party = c("KO", "TD", "ZP",
            "LEW", "KWIN", "BS", 
            "LIB"))
...
ggl_daily <- readRDS("../data/ggl_daily_spending.rds") %>% 
  ### TODO: CHANGE
  filter(str_detect(eur_amount, "EUR", negate = T)) %>% 
  select(-contains("party1"))
...
sum30_ggl <- ggl_all %>% 
    ...
    # distinct(internal_id, .keep_all = T) %>% 
    summarize(total_spend_formatted = sum(total_spend_formatted),
              total_num_ads = sum(readr::parse_number(`Number of Ads`)))
...
In total, political parties spent `r paste0(currency_symbol, scales::comma_format()(sum30_ggl$total_spend_formatted))` and ran `r sum30_ggl$total_num_ads` ad copies on Google in this timeframe.
...
In total, political parties spent `r paste0(currency_symbol, scales::comma_format()(sum7_ggl$total_spend_formatted))` and ran `r sum7_ggl$total_num_ads` ad copies on Google in this timeframe.
...
In total, political parties spent `r paste0(currency_symbol, scales::comma_format()(sum30_comb$total_spend_formatted))` and ran `r sum30_comb$total_num_ads` ad copies on digital platforms in this timeframe.
...
In total, political parties spent `r paste0(currency_symbol, scales::comma_format()(sum7_comb$total_spend_formatted))` and ran `r sum7_comb$total_num_ads` ads on digital platforms in this timeframe.
...
```{r, out.width="100%"}
more_data <- readr::read_rds("../lifelong/PL.rds")  %>%
    mutate(date_produced = lubridate::ymd(date)) %>%
...
hc_data_cumggl <-  ggl_daily  %>% 
  ...
  ungroup() #%>% 
  # filter(min(date_produced)!=date_produced)
