  ...
  plots <- gg %>%
    ...
    map(~ {
      ...
      hchart(.x,
        ...
        hc_plotOptions(bar = list(horizontal = TRUE, stacking = "normal")) %>%
        hc_yAxis(
          title = list(text = "% Party Budget spent on targeting method"),
          max = max(.x$perc)#,
          # min = 0
        ) %>%
        hc_xAxis(title = "", labels = list(style = list(fontSize = "10px"))) %>%
        ...
        highcharter::hc_tooltip(
          pointFormat = paste0(
            "<b>{point.value}</b><br>% of Total Budget Spent: {point.y:.1f}%<br>Spend a total of ",
            currency_symbol,
            "{point.spend_perlab} on this method."
          ),
          headerFormat = ""
        ) %>%
        hc_title(
...
contested_dat <- interest_targeting %>% 
  ...
  hc_dat %>% 
    ...
    hc_yAxis(title = list(text = "% of budget spent on targeting method"), labels = list(format = "{value}%"))%>%
            # hc_add_theme(hc_theme_google()) %>%
  highcharter::hc_tooltip(
    pointFormat = paste0("<b>{point.party}</b><br>..makes up {point.y:.1f}% of total budget spent on {point.value}. <br>..spend a total of ", currency_symbol, "{point.total_spend:.0f} on this audience.<br><br>Total Expenditure on audience is ", currency_symbol, "{point.total_spenderino:.0f}."),
    headerFormat = ""
  ) %>%
...
```{r}
thegraph30 <- get_contested_graph(election_dat30, minamount = 2000)
...
thegraph7 <- get_contested_graph(election_dat7, minamount = 1000)
...
region_dat <- election_dat30 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Poland")) 
